...
foreach ( $item_data as $item_d ) {
    ...
    $content .='<div class="clients_image">';
        $content .= (bool)$item_d['add_link'] ? '<a href="'.(!empty($url) ? esc_url($url) : '#').'" class="image_link">' : '<div class="image_wrapper">';
...
$output .= '<div class="integrio_module_clients clearfix'.esc_attr($clients_wrap_classes).'">';
    if ((bool)$use_carousel) {
